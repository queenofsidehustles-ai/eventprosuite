...
.pbh-help-head{background:linear-gradient(135deg,#4C1D95,#6D28D9);padding:14px 16px;display:flex;align-items:center;gap:10px;flex-shrink:0}
...
.pbh-msg.user{background:linear-gradient(135deg,#4C1D95,#6D28D9);color:#fff;border-bottom-right-radius:4px;align-self:flex-end}
.pbh-help-chips{padding:8px 10px;display:flex;flex-wrap:wrap;gap:5px;border-top:1px solid #EAE3ED;flex-shrink:0}
.pbh-chip{background:#fff;border:1.5px solid #DDD1E5;border-radius:999px;padding:4px 9px;font-size:11px;font-weight:600;color:#4C1D95;cursor:pointer;font-family:inherit;transition:all .15s;white-space:nowrap}
.pbh-chip:hover{background:#4C1D95;color:#fff;border-color:#4C1D95}
...
.pbh-help-input:focus{border-color:#8B5CF6}
.pbh-help-send{width:34px;height:34px;border-radius:9px;background:linear-gradient(135deg,#4C1D95,#6D28D9);color:#fff;border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:all .15s;font-family:inherit}
...
.pbh-dot{width:6px;height:6px;border-radius:50%;background:#8B5CF6;animation:pbh-bounce .7s infinite}
...
.pbh-support-link a{color:#6D28D9;font-weight:700;text-decoration:none}
...
`;
  ...
  const WELCOME_MSG = 'Hi! 👋 I can help you navigate Party Biz Hub. Ask me anything — or pick a quick question below.';

  function resetChat() {
    history = [];
    const msgs = document.getElementById('pbhHelpMsgs');
    msgs.innerHTML = `<div class="pbh-msg bot">${WELCOME_MSG}</div>`;
    document.getElementById('pbhHelpChips').style.display = 'flex';
    document.getElementById('pbhHelpInput').value = '';
  }
